...
export const analyzeIdentityDocument = async (imageBuffer) => {
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout: análisis excedió 30 segundos")), 30000));
        ...
        const [docResponseObj, visionResultObj, exifResult] = await analysisPromises;
        ...
        if (mrzLine && mrzLine.includes("<")) {
            ...
            const mrzLine2 = lines.find((l) => l.length >= 43 && !l.startsWith("P<") && l.includes("<<<<"));
            if (mrzLine2) {
            ...
            const nombreIndex = lines.findIndex((l) => /^NOMBRE|^nombre|^APELLIDO PATERNO|^apellido paterno/i.test(l.trim()));
            let fullNameParts = [];
        ...
        const foundState = stateKeywords.find((st) => fullText.includes(st));
        if (foundState) {
            state = foundState;
        }
